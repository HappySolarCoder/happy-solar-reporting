...
- roster_people_v1 (owner display_name via ghl_user_id)
...
def pipeline_name_lookup(db: firestore.Client, pipeline_ids) -> dict[str, str]:
    ...
    Store every id alias (compact_str, strip, raw str, snap.id, field id) so the
    include filter's raw str(pipelineId) still resolves to the pipeline name.
    """
    m: dict[str, str] = {}
    needed: list[str] = []
    seen: set[str] = set()
...
def user_name_lookup(db: firestore.Client, c: MetricContract) -> dict[str, str]:
    ...
    for doc in db.collection("roster_people_v1").stream():
        u = doc.to_dict() or {}
        name = compact_str(u.get("display_name")) or best_person_name(u)
        for key in {
            compact_str(u.get("ghl_user_id")),
            compact_str(u.get("ghlUserId")),
            compact_str(doc.id),
        }:
            if key and name:
                m[key] = name
...
def compute(db: firestore.Client, c: MetricContract, *, year: int, month: int, start: str | None = None, end: str | None = None, lead_source: str | None = None, pipeline_scope: str | None = None, setter_last_name: str | None = None) -> dict[str, Any]:
    ...
    setter_filter_norm = (str(setter_last_name).strip().lower() if setter_last_name else None)
    ...
    # createdAt is stored as ISO strings (e.g. 2026-08-01T12:14:35.991Z).
    # Range-query the NY window as date strings (widened one calendar day past end)
    # so format variants still match. Exact America/New_York compare stays below.
    ...
    opp_snaps = list(
        ...
        .where(c.created_at_field, ">=", start_bound)
        .where(c.created_at_field, "<", end_bound)
        .stream()
    )
    ...
    for doc in opp_snaps:
        ...
        # Resolve with the same aliases get_all stored (compact_str vs raw str).
    ...
    return {
        ...
        "breakdowns": {
            ...
            "created_by_owner": finalize_casefold_counts(by_owner, owner_labels),
            "created_by_setter_last_name": finalize_casefold_counts(by_setter, setter_labels),
